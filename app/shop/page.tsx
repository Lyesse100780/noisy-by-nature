...
<header className="absolute top-0 left-0 w-full z-20 flex justify-center md:justify-end items-center px-6 md:px-8 py-3 md:py-5">
  <nav className="flex space-x-6 md:space-x-8 text-[0.8rem] md:text-sm uppercase tracking-wider text-[#F5EBDD]/80 items-center font-body">
    <a href="/" className="hover:text-noisy-copper transition-colors">
      Home
    </a>
    <a href="/shop" className="hover:text-noisy-copper transition-colors">
      Shop
    </a>
    <a href="#contact" className="hover:text-noisy-copper transition-colors">
      Contact
    </a>
    ...
    >
      ...
      {/* --- HERO DU DROP --- */}
<div
  className="relative h-[20vh] md:h-[22vh] bg-cover bg-center flex flex-col items-center justify-center text-center"
  style={{
    backgroundImage: "url('/banner-western-floral.jpg')",
  }}
>
  <div className="absolute inset-0 bg-gradient-to-b from-[#1A1410]/60 via-[#1A1410]/40 to-[#1A1410]/60"></div>

  {/* Décalage du texte sur mobile */}
  <div className="relative z-10 px-4 mt-[3vh] md:mt-0">
    <h1 className="text-3xl md:text-5xl font-display mb-2 text-noisy-copper drop-shadow-[0_2px_6px_rgba(0,0,0,0.5)]">
      Western Floral — Drop 1/2
    </h1>
    <p className="text-sm md:text-lg text-[#E6D9C5]/80 tracking-wide">
      ...
      >
        {products.map((product) => (
          <a
            ...
            className="bg-[#241C17]/60 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 flex flex-col group"
          >
            <img
            ...
            <div className="p-6 flex flex-col flex-1 justify-between">
              <div>
                ...
                <p className="text-[#E6D9C5]/80 text-[15px] leading-relaxed mb-4 line-clamp-3">
                  {product.description.split("\n")[1] || ""}
                </p>
              </div>
              <div>
                <p className="text-lg font-semibold mb-3">{product.price}</p>
                <button className="bg-noisy-copper hover:bg-noisy-copper/80 text-white px-6 py-2 rounded-md transition-colors duration-300 w-full">
                  Buy Now
                </button>
              </div>
            </div>
